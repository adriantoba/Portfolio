...
class Shaders {
  ...
  public sunraysMaskShader: WebGLShader;
  public sunraysShader: WebGLShader;
  ...
  constructor(gl: WebGL2RenderingContext, ext: ExtraContext) {
    ...
    return max(0.2 * pow(color, vec3(0.416666667)) - 0.055, vec3(0));
    }
...
`
    ...
    this.sunraysMaskShader = Shader.compileShader(
      gl,
      gl.FRAGMENT_SHADER,
      `
    precision highp float;
...
`
    ...
    this.sunraysShader = Shader.compileShader(
      gl,
      gl.FRAGMENT_SHADER,
      `
    precision highp float;
...
`
    ...
    void main () {
        ...
        float pressure = (L + R + B + T - divergence) * 0.25; //0.25
